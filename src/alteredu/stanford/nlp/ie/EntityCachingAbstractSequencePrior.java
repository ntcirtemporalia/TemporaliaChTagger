...
import alteredu.stanford.nlp.sequences.SequenceListener;
...
public abstract class EntityCachingAbstractSequencePrior implements SequenceModel, SequenceListener {
  ...
  public EntityCachingAbstractSequencePrior(String backgroundSymbol, Index classIndex, List<FeatureLabel> doc) {
  ...
  public Entity extractEntity(int[] sequence, int position) {
    ...
    for ( ; position < sequence.length; position++) {
      if (sequence[position] == entity.type) {
        entity.words.add(doc.get(position).word());
  ...
  public boolean matches(Entity entity, int position) {
    if (doc.get(position).word().equalsIgnoreCase(entity.words.get(0))) {
      boolean matches = true;
      for (int j = 1; j < entity.words.size(); j++) {
        if (position + j >= doc.size()) {
          return false;
        }
        if (!doc.get(position+j).word().equalsIgnoreCase(entity.words.get(j))) {
          return false;
        }
      }
      return true;
  ...
  public void updateSequenceElement(int[] sequence, int position, int oldVal) {
    ...
    // are we joining 2 entities?
      ...
      newEntity.words.add(doc.get(position).word());
