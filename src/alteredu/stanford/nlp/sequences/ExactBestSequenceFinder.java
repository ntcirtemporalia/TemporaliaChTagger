...
public class ExactBestSequenceFinder implements BestSequenceFinder {
  ...
  public int[] bestSequence(SequenceModel ts) {
    ...
    for (int pos = leftWindow; pos < leftWindow + length; pos++) {
      //System.out.print("scoring word " + pos + " / " + (leftWindow + length) + ", productSizes =  " + productSizes[pos] + ", tagNum = " + tagNum[pos] + "...");
      //System.out.flush();
	...
	if((product % 350000) == 0) {
	  //System.out.printf("%.0f%% ", 100.0 * (float)product / (float)productSizes[pos]);
	  //System.out.flush();
	}
