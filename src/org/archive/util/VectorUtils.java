...
public class VectorUtils {
	...
	public static Vector<Double> Sum(Vector<Double> d1, double [] d2){
		if (d1.size() != d2.length) {
			System.out.println("ERROR: Vector Sum mismatch: " + d1.size() + " vs " + d2.length);
			new Exception().printStackTrace();
			System.exit(1);
		}
		Vector<Double> sum = new Vector<Double>();		
		for (int k = 0; k < d2.length; k++) {
			sum.add(d1.get(k) + d2[k]);			
		}
		return sum;
	}
	...
	public static Vector<Double> ScalarMultiply(Vector<Double> d, double val){
		Vector<Double> new_d = new Vector<Double>();		
		for (int k = 0; k < d.size(); k++) {
			new_d.add(d.get(k)*val);			
		}
		return new_d;		
	}
